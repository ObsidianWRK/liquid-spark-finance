...
export default defineConfig({
  ...
  workers: process.env.CI
    ? 1
    : process.env.HOOK_VALIDATION_MODE
      ? 1
      : undefined,
  ...
  globalTimeout: process.env.HOOK_VALIDATION_MODE
    ? 15 * 60 * 1000
    : 10 * 60 * 1000, // 15 min for hook validation
  ...
  use: {
    ...
    screenshot: process.env.HOOK_VALIDATION_MODE
      ? 'only-on-failure'
      : 'only-on-failure',

    /* Enhanced video recording for hook validation */
    video: process.env.HOOK_VALIDATION_MODE
      ? 'retain-on-failure'
      : 'retain-on-failure',
  ...
  projects: [
    /* Hook Validation Project - Chrome with enhanced debugging */
    {
      ...
      testMatch: [
        '**/hook-*.spec.ts',
        '**/comprehensive-hook-validation.spec.ts',
      ],
      use: {
    ...
    /* Mobile testing with hook validation */
    {
      ...
      testMatch: [
        '**/navigation-hook-validation.spec.ts',
        '**/performance-hook-validation.spec.ts',
      ],
      use: {
        ...devices['Pixel 5'],
        /* Mobile-specific settings for hook validation */
        launchOptions: {
          args: ['--disable-web-security', '--enable-logging', '--log-level=0'],
        },
      },
    },
