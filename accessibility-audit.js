...
const analyzeColorContrast = () => {
  ...
  const colorPairs = [
    // Text on backgrounds
    { fg: '#FFFFFF', bg: '#000000', context: 'Primary text on dark background' },
    { fg: 'rgba(235, 235, 245, 0.6)', bg: '#000000', context: 'Secondary text on dark background' },
    { fg: 'rgba(235, 235, 245, 0.3)', bg: '#000000', context: 'Tertiary text on dark background' },
    
    // Chart colors on dark background
    { fg: '#32D74B', bg: '#000000', context: 'Income color on dark background' },
    { fg: '#FF453A', bg: '#000000', context: 'Spending color on dark background' },
    { fg: '#0A84FF', bg: '#000000', context: 'Savings color on dark background' },
    { fg: '#BF5AF2', bg: '#000000', context: 'Investments color on dark background' },
    { fg: '#FF9F0A', bg: '#000000', context: 'Debt color on dark background' },
    
    // Interactive states
    { fg: '#007AFF', bg: 'rgba(0, 122, 255, 0.1)', context: 'Focus indicator' },
    { fg: '#FFFFFF', bg: 'rgba(255, 255, 255, 0.15)', context: 'Selected state background' },
    
    // Tooltip colors
    { fg: '#FFFFFF', bg: 'rgba(0, 0, 0, 0.9)', context: 'Tooltip text' },
    { fg: 'rgba(255, 255, 255, 0.6)', bg: 'rgba(0, 0, 0, 0.9)', context: 'Tooltip secondary text' },
  ];
  ...
  colorPairs.forEach(({ fg, bg, context }) => {
    ...
    console.log(`   Contrast: ${contrast.toFixed(2)}:1 (AA: ${wcagAA ? 'PASS' : 'FAIL'}, AAA: ${wcagAAA ? 'PASS' : 'FAIL'})`);
...
const analyzeKeyboardNavigation = () => {
  ...
  const keyboardPatterns = [
    ...
    },
    {
      ...
      implementation: 'PARTIAL - Basic focus management, needs data point navigation',
  ...
  keyboardPatterns.forEach(({ component, expectedBehavior, implementation, status }) => {
...
const analyzeAppleAccessibility = () => {
  ...
  const appleFeatures = [
    ...
    },
    {
      ...
      implementation: 'PARTIAL - Good base colors, needs high contrast adaptation',
      status: '⚠️  NEEDS IMPROVEMENT'
    },
    {
...
const generateReport = (contrastResults, keyboardResults, ariaResults, appleResults) => {
  ...
  console.log(`   Color Contrast Issues: ${contrastResults.filter(r => !r.wcagAA).length}`);
  console.log(`   Keyboard Navigation Issues: ${keyboardResults.filter(r => r.status.includes('❌')).length}`);
  console.log(`   ARIA Implementation Issues: ${ariaResults.filter(r => r.status.includes('❌')).length}`);
  console.log(`   Apple Accessibility Issues: ${appleResults.filter(r => r.status.includes('❌')).length}\n`);
  ...
  console.log('   • Ensure essential information is not conveyed through motion alone\n');
...
const runAccessibilityAudit = async () => {
  ...
  console.log('Component Scope: Chart Components (GraphBase, LineChart, AreaChart, StackedBarChart, TimeRangeToggle)');
  ...
  return {
    summary: {
      totalIssues: contrastResults.filter(r => !r.wcagAA).length + 
      ...
      keyboardIssues: keyboardResults.filter(r => r.status.includes('❌')).length,
...
export { runAccessibilityAudit, analyzeColorContrast, analyzeKeyboardNavigation, analyzeARIA, analyzeAppleAccessibility };
