...
test.describe('Safe Area Dock Tests', () => {
  ...
  test('Dock should respect safe area insets on iPhone 15', async ({ page }) => {
  ...
  test('Dock should not overlap with viewport content', async ({ page }) => {
    ...
    // Get dock z-index
    const dockContainer = await page.locator('.fixed.bottom-0.left-0.right-0.z-50');
    const dockZIndex = await dockContainer.evaluate(el => 
  ...
  test('FAB should be positioned correctly with safe areas', async ({ page }) => {
    ...
    if (fabBounds && viewportSize) {
      ...
      expect(fabBounds.x + fabBounds.width).toBeLessThanOrEqual(viewportSize.width);
    }
  });
});
...
test.describe('Safe Area Dock Tests - Landscape', () => {
  ...
  test('Dock should remain flush in landscape orientation', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
