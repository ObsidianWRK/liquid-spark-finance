...
test.describe('Quick Access Cards - Tablet Viewport', () => {
  ...
  test('should align Quick Access cards properly on tablet view', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="account-card"], .relative.rounded-2xl', { timeout: 10000 });
    
    // Get all account cards
    const accountCards = await page.locator('[data-testid="account-card"], .relative.rounded-2xl').all();
  ...
  test('should maintain grid layout without gaps', async ({ page }) => {
    ...
    if (cards.length >= 2) {
      ...
      if (firstCardBox && secondCardBox) {
        ...
        expect(Math.abs(firstCardBox.height - secondCardBox.height)).toBeLessThan(10);
      }
    }
  });
}); 