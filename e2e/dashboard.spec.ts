...
test.describe('Dashboard E2E Tests', () => {
  ...
  test('should display transaction list with proper data', async ({ page }) => {
    ...
    await expect(
      firstTransaction.locator('[data-testid="transaction-merchant"]')
    ).toBeVisible();
    await expect(
      firstTransaction.locator('[data-testid="transaction-amount"]')
    ).toBeVisible();
    await expect(
      firstTransaction.locator('[data-testid="transaction-date"]')
    ).toBeVisible();
  });
  ...
  test('should display account details correctly', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(
        card.locator('[data-testid="account-balance"]')
      ).toBeVisible();
