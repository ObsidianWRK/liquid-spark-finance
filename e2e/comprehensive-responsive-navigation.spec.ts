...
test.describe('Comprehensive Responsive Navigation', () => {
  ...
  test('should navigate through every main navigation item successfully', async () => {
    ...
    for (const navItem of mainNavItems) {
      ...
      if (await navButton.count() === 0) {
        console.warn(`⚠️ Navigation item "${navItem.name}" not found, trying alternative selectors...`);
  ...
  test('should return to dashboard from all navigation states', async () => {
    ...
    for (const testPage of testPages) {
      console.log(`🔄 Testing round-trip: Dashboard → ${testPage.name} → Dashboard`);
      ...
      console.log(`✅ Successfully returned to dashboard from ${testPage.name}`);
    }
  });
  ...
  test('should handle responsive layout transitions smoothly', async () => {
    ...
    for (const size of sizes) {
      ...
      console.log(`✅ Layout transition to ${size.width}x${size.height} successful`);
    }
  });
  ...
  test('should be fully responsive on Desktop viewports', async ({ page }) => {
    ...
    for (const viewport of desktopViewports) {
      ...
      console.log(`✅ ${viewport.width}x${viewport.height} responsive test passed`);
    }
  });
  ...
  test('should handle layout transitions smoothly', async ({ page }) => {
    ...
    for (const size of sizes) {
      ...
      console.log(`✅ Layout transition successful for ${size.width}x${size.height}`);
    }
  });
