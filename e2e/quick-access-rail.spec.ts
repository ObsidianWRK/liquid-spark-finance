...
test.describe('Quick Access Rail - Comprehensive Testing', () => {
  ...
  test.describe('Mobile Viewport (375px)', () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.goto('/');
      await page.waitForSelector('[data-quick-access-rail]', {
        timeout: 10000,
      });
    });

    test('should render horizontal scrolling rail on mobile', async ({
      page,
    }) => {
    ...
    test('should display navigation arrows when scrollable', async ({
      page,
    }) => {
      const rail = page.locator('[data-quick-access-rail]');
    ...
    test('should scroll horizontally when arrow buttons are clicked', async ({
      page,
    }) => {
      const rail = page.locator('[data-quick-access-rail]');
      const scrollContainer = rail.locator('.overflow-x-auto');
    ...
    test('should support touch swipe gestures', async ({ page }) => {
      const scrollContainer = page.locator(
        '[data-quick-access-rail] .overflow-x-auto'
      );
  ...
  test.describe('Desktop Viewport (1920px)', () => {
    ...
    test('should display View All button when accounts exceed limit', async ({
      page,
    }) => {
      const rail = page.locator('[data-quick-access-rail]');
      ...
      const accountCount = parseInt(
        subtitleText?.match(/(\d+) accounts/)?.[1] || '0'
      );

      if (accountCount > 6) {
        // maxVisibleDesktop is set to 6
    ...
    test('should maintain proper card sizing in grid layout', async ({
      page,
    }) => {
      const rail = page.locator('[data-quick-access-rail]');
  ...
  test.describe('Balance Visibility Toggle', () => {
    ...
    test('should toggle balance visibility', async ({ page }) => {
      const rail = page.locator('[data-quick-access-rail]');
      const toggleButton = rail.locator(
  ...
  test.describe('Performance', () => {
    test('should load Quick Access Rail within performance budget', async ({
      page,
    }) => {
  ...
  test.describe('Edge Cases', () => {
    ...
    test('should handle long account names properly', async ({ page }) => {
      ...
      const cards = page.locator(
        '[data-quick-access-rail] .min-w-\\[160px\\], [data-quick-access-rail] .w-full'
      );
      ...
      if (hasOverflow) {
        const computedStyle = await firstCard.evaluate(
