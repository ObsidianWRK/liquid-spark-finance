...
test.describe('Financial Planning - Comprehensive Testing', () => {
  ...
  viewports.forEach(({ name, width, height }) => {
    test.describe(`${name} (${width}x${height})`, () => {
      ...
      test('should load planning page without errors', async ({ page }) => {
        ...
        await expect(
          page.locator(
            'text=Set goals, track progress, and plan for your financial future'
          )
        ).toBeVisible();
      });
      ...
      test('should display charts and visualizations', async ({ page }) => {
        ...
        await expect(
          page.locator('text=Retirement Savings Projection')
        ).toBeVisible();
      });
  ...
  test.describe('Data Validation', () => {
    ...
    test('should display realistic financial data', async ({ page }) => {
      ...
      const savingsAmount = page
        .locator('text=Current Savings')
        .locator('..')
        .locator('text=/\\$[0-9,]+/');
