...
test.describe('Mobile Responsive Financial Pages', () => {
  ...
  for (const viewport of viewports) {
    test.describe(`${viewport.name} (${viewport.width}x${viewport.height})`, () => {
      ...
      test('Credit Score page mobile responsive layout', async ({ page }) => {
        ...
        await page.waitForSelector('h1:has-text("Credit Score")', {
          timeout: 10000,
        });
        ...
        const backButtonHeight = await backButton.evaluate(
          (el) => el.getBoundingClientRect().height
        );
        expect(backButtonHeight).toBeGreaterThanOrEqual(44);
      });

      test('Investment Tracker page mobile responsive layout', async ({
        page,
      }) => {
        await page.goto('/investments');
        await page.waitForSelector('h1:has-text("Investment Tracker")', {
          timeout: 10000,
        });
      ...
      test('Financial Planning page mobile responsive layout', async ({
        page,
      }) => {
        await page.goto('/planning');
        await page.waitForSelector('h1:has-text("Financial Planning")', {
          timeout: 10000,
        });
  ...
  test('Accessibility: Touch targets meet minimum size requirements', async ({
    page,
  }) => {
  ...
  test('Content readability on small screens', async ({ page }) => {
    ...
    for (const pagePath of pages) {
      ...
      const contentElements = page.locator(
        '.bg-white\\/\\[0\\.02\\], .bg-gradient-to-r, .grid'
      );
