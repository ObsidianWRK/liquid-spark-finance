...
test.describe('Final Comprehensive Navigation Test', () => {
  test('should navigate through EVERY menu item and successfully return to dashboard', async ({ page }) => {
    ...
    const initialContent = await page.evaluate(() => document.body.textContent?.trim().length || 0);
    ...
    for (const navItem of mainNavItems) {
      ...
      let navButton = page.locator(`button:has-text("${navItem.name}")`).first();
      ...
      if (await navButton.count() > 0) {
        ...
        const contentLength = await page.evaluate(() => document.body.textContent?.trim().length || 0);
        ...
        console.log(`✅ Successfully returned to dashboard from ${navItem.name}`);
      } else {
    ...
    for (const viewRoute of viewBasedRoutes) {
      ...
      if (await homeButton.count() > 0) {
        ...
        await page.waitForTimeout(2000);
        console.log(`✅ Successfully returned to dashboard via Home button from ${viewRoute.name}`);
      } else {
        // Direct navigation to home
        await page.goto('/', { waitUntil: 'networkidle' });
        await page.waitForTimeout(1500);
        console.log(`✅ Successfully returned to dashboard via direct navigation from ${viewRoute.name}`);
      }
    }
