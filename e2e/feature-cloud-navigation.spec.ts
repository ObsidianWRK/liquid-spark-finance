...
test.describe('FeatureCloud Navigation', () => {
  ...
  const features = [
    ...
    { text: 'AI Insights', route: 'insights', expectedTitle: 'Financial Insights' },
    { text: 'Investment', route: 'investments', expectedTitle: 'Investment Portfolio' },
    { text: 'Budgeting', route: 'budget', expectedTitle: 'Budget Planner' },
    { text: 'Goals', route: 'savings', expectedTitle: 'Savings Goals' },
    { text: 'Analytics', route: 'analytics', expectedTitle: 'Financial Analytics Dashboard' },
    { text: 'Security', route: 'dashboard', expectedTitle: 'dashboard' }, // Security goes to dashboard
    { text: 'Planning', route: 'planning', expectedTitle: 'Financial Planning' },
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('text=Loading your financial dashboard...', { state: 'hidden', timeout: 30000 });
    
    // Wait for page to be fully loaded
    await page.waitForLoadState('networkidle');
    
    // Wait for FeatureCloud to be visible
    await page.waitForSelector('h1:has-text("Intelligence you can bank on")', { state: 'visible', timeout: 30000 });
    
    // Additional wait to ensure all buttons are rendered
    await page.waitForTimeout(1000);
  });

  test('FeatureCloud component is visible and contains all buttons', async ({ page }) => {
  ...
  for (const feature of features) {
    test(`${feature.text} button navigates to ${feature.route}`, async ({ page }) => {
      ...
      if (feature.route === 'dashboard') {
        ...
        await expect(page.locator('text=Intelligence you can bank on')).toBeVisible({ timeout: 10000 });
      } else {
        ...
        switch (feature.route) {
          case 'accounts':
            ...
            break;
          case 'insights':
            await expect(page.locator('text=Financial Insights')).toBeVisible({ timeout: 10000 });
            break;
          case 'investments':
            await expect(page.locator('text=Investment')).toBeVisible({ timeout: 10000 });
            break;
          case 'budget':
            await expect(page.locator('text=Budget')).toBeVisible({ timeout: 10000 });
            break;
          case 'savings':
            await expect(page.locator('text=Savings Goals')).toBeVisible({ timeout: 10000 });
            break;
          case 'analytics':
          ...
          case 'planning':
            ...
            break;
          case 'credit':
            await expect(page.locator('text=Credit Score')).toBeVisible({ timeout: 10000 });
            break;
        }
      }
  ...
  test('All buttons have proper keyboard accessibility', async ({ page }) => {
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      if (tagName === 'BUTTON') {
        ...
        const isFeatureButton = features.some(f => buttonText?.includes(f.text));
        if (isFeatureButton) {
  ...
  test('Mobile responsiveness - buttons are properly sized', async ({ page }) => {
  ...
  test('Desktop responsiveness - buttons scale appropriately', async ({ page }) => {
