...
test.describe('FeatureCloud Quick Test', () => {
  test('Verify all FeatureCloud buttons are clickable and navigate correctly', async ({ page }) => {
    ...
    for (const button of buttons) {
      try {
        ...
        await page.waitForSelector('h1:has-text("Intelligence you can bank on")', { state: 'visible', timeout: 10000 });
        ...
        if (url.includes(`tab=${button.expectedRoute}`)) {
          console.log(`✅ ${button.text} button works - navigates to ${button.expectedRoute}`);
          successCount++;
        } else {
          console.log(`❌ ${button.text} button failed - expected ${button.expectedRoute}, got ${url}`);
        }
      } catch (error) {
