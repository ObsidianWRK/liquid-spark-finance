...
test.describe('Critical User Journeys E2E Tests', () => {
  ...
  test.describe('New User Onboarding Journey', () => {
    ...
    test('should guide user through adding first account', async ({ page }) => {
      ...
      if (await addAccountCTA.isVisible()) {
        ...
        await expect(page.locator('[data-testid="account-setup-modal"]')).toBeVisible();
        ...
        await expect(page.locator('[data-testid="account-setup-modal"]')).not.toBeVisible();
        
        // Account should appear on dashboard
        await expect(page.locator('[data-testid="account-card"]')).toBeVisible();
  ...
  test.describe('Daily Financial Management Journey', () => {
    test('should complete transaction entry and categorization workflow', async ({ page }) => {
      ...
      await page.hover('[data-testid="transaction-item"]:has-text("Whole Foods")');
    ...
    test('should filter and search transactions', async ({ page }) => {
      ...
      const visibleTransactions = page.locator('[data-testid="transaction-item"]');
      ...
      const groceryTransactions = page.locator('[data-testid="transaction-item"]');
  ...
  test.describe('Budget Planning Journey', () => {
    ...
    test('should show budget alerts when approaching limits', async ({ page }) => {
      await page.goto('/budget');
    ...
    test('should generate budget reports', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="budget-report-chart"]')).toBeVisible();
  ...
  test.describe('Financial Insights Journey', () => {
    test('should explore comprehensive financial insights', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="health-score-card"]')).toBeVisible();
      await expect(page.locator('[data-testid="eco-score-card"]')).toBeVisible();
      await expect(page.locator('[data-testid="financial-score-card"]')).toBeVisible();
    ...
    test('should switch between different insight views', async ({ page }) => {
      ...
      for (const view of viewButtons) {
        ...
        if (await button.isVisible()) {
          await button.click();
          await expect(page.locator(`[data-testid="${view}-content"]`)).toBeVisible();
        }
      }
    });
    ...
    test('should customize insight preferences', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="projections-section"]')).toBeVisible();
    });
  });
