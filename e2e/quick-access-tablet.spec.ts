...
test.describe('Quick Access Cards - Tablet Viewport', () => {
  ...
  test('should align Quick Access cards properly on tablet view', async ({ page }) => {
    // Wait for cards to load
    await page.waitForSelector('[data-testid="account-card"], .account-card', { timeout: 10000 });
    
    // Get all account cards
    const accountCards = await page.locator('[data-testid="account-card"], .account-card').all();
  ...
  test('should maintain grid layout without gaps', async ({ page }) => {
    ...
    const cards = await gridContainer.locator('[data-testid="account-card"], .account-card').all();
  ...
  test('should display tooltips for truncated text', async ({ page }) => {
    ...
    const accountNameElements = await page.locator('[data-testid="account-name"], .account-name').all();
    
    for (const element of accountNameElements) {
      // Check if text is truncated (scrollWidth > clientWidth)
      const isOverflowing = await element.evaluate(el => el.scrollWidth > el.clientWidth);
  ...
  test('should maintain accessibility standards', async ({ page }) => {
    ...
    const cards = await page.locator('[data-testid="account-card"], .account-card').all();
