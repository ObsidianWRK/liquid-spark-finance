...
import {
  HookValidationMonitor,
  NavigationTester,
  ComponentValidator,
  TEST_CONFIG,
} from './hook-validation-config';
...
test.describe('React Hook Violations Detection', () => {
  ...
  test('should navigate to all tabs without hook violations', async ({
    page,
  }) => {
    for (const tab of TEST_CONFIG.tabs) {
      ...
      try {
        ...
        expect(
          errors,
          `Hook violations found on ${tab} tab: ${errors.join(', ')}`
        ).toHaveLength(0);
  ...
  test('should handle rapid navigation without hook violations', async ({
    page,
  }) => {
    const rapidTabs = [
      'dashboard',
    ...
    expect(
      errors,
      `Hook violations during rapid navigation: ${errors.join(', ')}`
    ).toHaveLength(0);
  });

  test('should handle URL parameter navigation without hook violations', async ({
    page,
  }) => {
    for (const tab of ['dashboard', 'accounts', 'transactions', 'profile']) {
      ...
      expect(
        errors,
        `Hook violations with URL param navigation to ${tab}: ${errors.join(', ')}`
      ).toHaveLength(0);
    }
  });

  test('should handle page refresh without hook violations', async ({
    page,
  }) => {
    ...
    expect(
      errors,
      `Hook violations after page refresh: ${errors.join(', ')}`
    ).toHaveLength(0);
  });
  ...
  test('should validate Index.tsx hook order fix', async ({ page }) => {
    ...
    const errors = hookMonitor.getErrors();
    const hookOrderErrors = errors.filter(
      (error) =>
        error.includes('rendered more hooks') || error.includes('hook order')
    );

    expect(
      hookOrderErrors,
      `Hook order violations detected: ${hookOrderErrors.join(', ')}`
    ).toHaveLength(0);
  });

  test('should validate PerformanceMonitor component hook fix', async ({
    page,
  }) => {
    // Check if performance monitor is enabled (development mode)
    const perfMonitor = page.locator(
      '[data-testid="performance-monitor"], .performance-monitor'
    );
    ...
    if ((await perfMonitor.count()) > 0) {
      ...
      expect(
        errors,
        `PerformanceMonitor hook violations: ${errors.join(', ')}`
      ).toHaveLength(0);
    }
  });

  test('should handle error boundary scenarios without hook violations', async ({
    page,
  }) => {
    ...
    expect(
      errors,
      `Hook violations in error boundary: ${errors.join(', ')}`
    ).toHaveLength(0);
  });
  ...
  test('should validate component mounting/unmounting', async ({ page }) => {
    ...
    for (const tab of tabs) {
      ...
      expect(
        errors,
        `Hook violations during ${tab} mount/unmount: ${errors.join(', ')}`
      ).toHaveLength(0);
    }
  });

  test('should validate Suspense boundaries work correctly', async ({
    page,
  }) => {
    ...
    expect(
      errors,
      `Hook violations with Suspense boundaries: ${errors.join(', ')}`
    ).toHaveLength(0);
  });
  ...
  test('should validate context providers are working', async ({ page }) => {
    ...
    expect(
      errors,
      `Hook violations with context providers: ${errors.join(', ')}`
    ).toHaveLength(0);
  });
});
