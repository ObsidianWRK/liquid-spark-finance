...
async function createChartTestPage(
  page: Page,
  chartType: string,
  data: any[],
  options: any = {}
  ...
  await page.waitForSelector(
    '[data-testid="chart-container"], .chart-component',
    { timeout: 10000 }
  );
...
test.describe('Desktop Chart Components - Comprehensive Test Suite', () => {
      ...
      test.describe(`${viewport.name} (${viewport.width}x${viewport.height})`, () => {
        ...
        test('GraphBase foundation renders correctly', async ({ page }) => {
          ...
          await compareScreenshot(
            page,
            `graphbase-${viewport.name.toLowerCase().replace(' ', '-')}`
          );
        });
        ...
        test('LineChart renders with proper styling', async ({ page }) => {
          ...
          const strokeWidth = await firstLine.evaluate(
          ...
          await compareScreenshot(
            page,
            `linechart-${viewport.name.toLowerCase().replace(' ', '-')}`
          );
        });
        ...
        test('AreaChart gradient rendering', async ({ page }) => {
          ...
          const fillOpacity = await firstArea.evaluate(
            (el) => getComputedStyle(el).fillOpacity
          );
          ...
          await compareScreenshot(
            page,
            `areachart-${viewport.name.toLowerCase().replace(' ', '-')}`
          );
        });

        test('StackedBarChart category interactions', async ({ page }) => {
          await createChartTestPage(
            page,
            'stackedBar',
            CHART_TEST_DATA.lineChart
          );
          ...
          await compareScreenshot(
            page,
            `stackedbarchart-${viewport.name.toLowerCase().replace(' ', '-')}`
          );
        });
        ...
        test('TimeRangeToggle selection behavior', async ({ page }) => {
          ...
          const threeMonthButton = page.locator('[role="tab"]', {
            hasText: '3M',
          });
          ...
          const isSelected =
            await threeMonthButton.getAttribute('aria-selected');
  ...
  test.describe('Desktop-Specific Features', () => {
    ...
    test('Multi-chart coordination with global time range', async ({
      page,
    }) => {
      ...
      const firstChartTimeButton = firstChart.locator('[role="tab"]', {
        hasText: '3M',
      });
      await firstChartTimeButton.click();

      // Verify both charts update
      const allTimeButtons = page.locator(
    ...
    test('High-resolution rendering quality', async ({ page }) => {
      ...
      const fontSize = await firstLabel.evaluate(
  ...
  test.describe('Apple UX Validation', () => {
    ...
    test('Color accuracy (Apple system colors)', async ({ page }) => {
      ...
      const chartContainer = page.locator('.chart-component');
      const backgroundStyle = await chartContainer.evaluate(
        (el) => getComputedStyle(el).background
      );

      // Should contain blue/purple gradient colors
      expect(backgroundStyle).toMatch(
        /(rgb\(30, 58, 138\)|rgb\(55, 48, 163\)|rgb\(88, 28, 135\))/
      );
      ...
      const strokeColor = await chartLine.evaluate(
    ...
    test('Typography rendering (SF Pro fallbacks)', async ({ page }) => {
      ...
      if ((await axisLabels.count()) > 0) {
        const labelFont = await axisLabels
          .first()
      ...
      // Check font weights
      const titleWeight = await title.evaluate(
    ...
    test('Focus states and accessibility', async ({ page }) => {
      ...
      // Check focus ring
      const buttonFocusStyle = await timeButton.evaluate(
  ...
  test.describe('Performance and Accessibility', () => {
    ...
    test('High contrast and reduced motion support', async ({ page }) => {
      ...
      const backgroundColor = await title.evaluate(
