...
test.describe('TopBar Visibility', () => {
  ...
  test('TopBar should be visible on homepage at 1024px viewport', async ({ page }) => {
  ...
  test('TopBar should contain search and action buttons', async ({ page }) => {
    ...
    await expect(topBar.locator('button[aria-label="Add new transaction"]')).toBeVisible();
    await expect(topBar.locator('button[aria-label="View notifications"]')).toBeVisible();
    await expect(topBar.locator('button[aria-label="Open settings"]')).toBeVisible();
    await expect(topBar.locator('button[aria-label="View profile"]')).toBeVisible();
  });
  ...
  test('TopBar should be visible on large desktop viewport', async ({ page }) => {
  ...
  test('TopBar should respond to scroll behavior (no early hide)', async ({ page }) => {
    await page.goto('http://localhost:5173');
    await page.waitForLoadState('networkidle');
