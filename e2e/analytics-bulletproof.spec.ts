...
test.describe('Analytics Tab - Bulletproof Implementation', () => {
  ...
  test('✅ Analytics tab loads without destructuring crashes', async ({
    page,
  }) => {
    ...
    const destructuringErrors = errors.filter(
      (error) =>
    ...
    await expect(
      page.locator(
        '[data-testid=\"financial-metrics\"], .financial-metrics, h2:has-text(\"Financial\")'
      )
    ).toBeVisible();
  ...
  test('🌐 Analytics tab works under slow network conditions', async ({
    page,
  }) => {
  ...
  test('🔄 Analytics tab handles rapid navigation without crashes', async ({
    page,
  }) => {
  ...
  test('⚠️ Error boundary catches and recovers from errors', async ({
    page,
  }) => {
  ...
  test('🔍 Data validation prevents malformed data crashes', async ({
    page,
  }) => {
  ...
  test('⏱️ Timeout handling prevents infinite loading', async ({ page }) => {
    ...
    const timeoutErrors = errors.filter(
      (error) => error.includes('destructur') || error.includes('timeout')
    );

    // Timeout errors are OK, destructuring errors are not

    const destructuringErrors = timeoutErrors.filter((e) =>
      e.includes('destructur')
    );
    expect(destructuringErrors).toHaveLength(0);
