...
test.describe('Back Navigation', () => {
  ...
  test('should navigate back through history stack correctly', async ({
    page,
  }) => {
    // Start at dashboard
    ...
    await page.click(
      '[data-testid="account-card"], .cursor-pointer:has(h3):first'
    );
    await page.waitForLoadState('networkidle');
  ...
  test('should handle deep links with fallback navigation', async ({
    page,
  }) => {
  ...
  test('should work correctly from dashboard account cards', async ({
    page,
  }) => {
    // Start at dashboard
    await expect(page).toHaveURL('/');
