...
test.describe('Calculator Hub', () => {
  test('displays all calculator cards with proper design elements', async ({
    page,
  }) => {
    ...
    const calculatorCards = page
      .locator('[data-testid="calculator-card"]')
      .or(
        page
          .locator('.bg-white\\/\\[0\\.02\\]')
          .filter({ hasText: 'Compound Interest' })
      );
  ...
  test('calculator card interactions work correctly', async ({ page }) => {
    ...
    const compoundInterestCard = page
      .locator('text=Compound Interest')
      .locator('..');
  ...
  test('dropdown menu functionality works', async ({ page }) => {
    ...
    const moreButton = firstCard
      ...
      .or(firstCard.locator('button').filter({ hasText: '' }));
  ...
  test('navigation to individual calculators works', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Compound Interest Calculator')
    ).toBeVisible();
  });
