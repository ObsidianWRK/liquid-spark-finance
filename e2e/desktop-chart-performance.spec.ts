...
async function measurePerformanceMetrics(
  ...
  const metrics = await page.evaluate(() => {
    ...
    return {
      ...
      firstPaint:
        paintMetrics.find((m) => m.name === 'first-paint')?.startTime || 0,
      firstContentfulPaint:
        paintMetrics.find((m) => m.name === 'first-contentful-paint')
          ?.startTime || 0,
...
async function monitorAnimationFrameRate(
  page: Page,
  duration = 2000
...
async function createPerformanceTestPage(
  page: Page,
  chartType: string,
  data: unknown[],
  ...
  await page.evaluate(
    ({ chartType, data, enableAnimations }: any) => {
      // Create container
...
test.describe('Desktop Chart Performance Tests', () => {
  test.describe('Load Time Performance', () => {
    ...
    test('Chart loads within performance threshold', async ({ page }) => {
      const metrics = await measurePerformanceMetrics(page, async () => {
        await createPerformanceTestPage(
          page,
          'line',
          LARGE_CHART_DATA.slice(0, 100)
        );
      });

      // Validate load time
      expect(metrics.totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.LOAD_TIME_MAX
      );

      // Validate first paint
      expect(metrics.firstPaint).toBeLessThan(
        PERFORMANCE_THRESHOLDS.PAINT_TIME_MAX
      );

      // Validate interactive time
      expect(metrics.domInteractive).toBeLessThan(
        PERFORMANCE_THRESHOLDS.INTERACTIVE_TIME_MAX
      );
    ...
    test('Large dataset handling (1000+ points)', async ({ page }) => {
      ...
      expect(metrics.totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.LOAD_TIME_MAX * 2
      );
    ...
    test('Multiple chart types load performance comparison', async ({
      page,
    }) => {
  ...
  test.describe('Animation Performance', () => {
    ...
    test('60fps animation target', async ({ page }) => {
      ...
      expect(frameMetrics.droppedFrames).toBeLessThan(
        frameMetrics.frames * 0.1
      ); // Less than 10% dropped
    ...
    test('Event listener cleanup', async ({ page }) => {
      ...
      const initialListeners = await page.evaluate(() => {
        ...
        events.forEach((eventType) => {
          const listeners =
            (document as any).getEventListeners?.(document.body)?.[eventType] ||
            [];
