    ...
    for (const viewport of desktopViewports) {
      ...
      for (const route of allPageRoutes) {
        ...
        try {
          ...
          console.log(`    ✅ ${route.name}: Layout valid, ${layoutMetrics.contentElements} elements, ${interactiveCheck.visibleButtons} buttons`);
  ...
  test('should validate consistent layout components across all pages', async ({ page }) => {
    ...
    for (const route of allPageRoutes) {
      ...
      try {
        ...
        console.log(`  ✅ ${route.name}: ${layoutAnalysis.totalElements} elements, ${layoutAnalysis.interactiveElements} interactive, responsive: ${layoutAnalysis.hasResponsiveClasses}`);
    ...
    console.log(`✅ Layout consistency validated: ${layoutResults.length} pages analyzed, ${responsivePages} responsive`);
  });
    ...
    for (const route of criticalPages) {
      ...
      try {
        ...
        for (const viewport of transitionViewports) {
          ...
          console.log(`    ✅ ${viewport.width}px: No overflow, content visible`);
        }
