...
async function reproduceAnalyticsCrash() {
  ...
  try {
    ...
    try {
      ...
      const analyticsButton = await page.$('[data-tab="analytics"]') || 
    ...
    console.log('⏳ Waiting 15 seconds for delayed crash (monitoring for errors)...');
    ...
    while (Date.now() - startTime < 15000 && !crashDetected) {
      ...
      const hasDestructuringError = logs.some(log => 
      ...
      const reactErrors = await page.evaluate(() => {
        ...
        const errorMessages = Array.from(errorBoundaries).map(el => el.textContent);
      ...
      console.log(`⏱️  ${Math.floor((Date.now() - startTime) / 1000)}s elapsed...`);
    }
    ...
    console.log(crashDetected ? '💥 CRASH REPRODUCTION SUCCESSFUL!' : '✅ No crash detected in 15 seconds');
