...
const TransactionWithScores = ({ transaction, scores, currency }: TransactionWithScoresProps) => {
  ...
  const getScoreIcon = (type: 'financial' | 'health' | 'eco', score: number) => {
    ...
    switch (type) {
      case 'financial':
        return score >= 60 ? 
          <TrendingUp className={cn(iconClass, "text-green-400")} /> :
          <TrendingDown className={cn(iconClass, "text-red-400")} />;
      case 'health':
        return <Heart className={cn(iconClass, score >= 60 ? "text-pink-400" : "text-gray-400")} />;
      case 'eco':
        return <Leaf className={cn(iconClass, score >= 60 ? "text-green-400" : "text-gray-400")} />;
    }
  };
  ...
  return (
    <GlassCard 
      className="glass-card glass-dark hover:scale-[1.02] transition-all duration-300 cursor-pointer group p-4 mb-3"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <div 
              className={cn(
                "score-indicator bg-gradient-to-br",
