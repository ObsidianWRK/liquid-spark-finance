...
const StackedBarTooltip: React.FC<{
  ...
  ({
    ...
    const formatValue = useCallback(
      (value: number, type: 'currency' | 'percentage' | 'number') => {
        switch (type) {
          case 'currency':
...
export const StackedBarChart = forwardRef<ChartRef, StackedBarChartProps>(
  (
    ...
    const computedSeries = useMemo((): ChartSeries[] => {
      ...
      if (groupSmallCategories) {
        const categoryTotals = categories.map((key) => ({
          key,
          total: data.reduce(
            (sum, point) => sum + ((point[key] as number) || 0),
            0
          ),
        }));
        ...
        if (point) {
          ...
          if (firstSeries) {
            onBarClick(
              point,
              firstSeries.dataKey,
              point[firstSeries.dataKey] as number
            );
          }
        }
      },
    ...
    const cssProperties = useMemo(() => generateGraphCSSProperties('dark'), []);
