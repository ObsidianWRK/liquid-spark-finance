  ...
  (
    ...
    const areaConfigMerged = useMemo(
      () => ({
        ...
        portfolioMode:
          areaConfig.portfolioMode ?? financialType === 'allocation',
      }),
    ...
    const processedData = useMemo(() => {
      ...
      return data.map((point) => {
        const numericKeys = Object.keys(point).filter(
          (key) =>
            key !== 'date' && key !== 'label' && typeof point[key] === 'number'
        );
    ...
    const optimizedSeries = useMemo((): ChartSeries[] => {
      ...
      processedData,
      series,
      areaConfigMerged.strokeWidth,
      areaConfigMerged.fillOpacity,
      multiSeries,
      seriesColors,
    ]);
    ...
    const portfolioSummary = useMemo(() => {
      if (
        !areaConfigMerged.portfolioMode ||
        !optimizedData.length ||
        !optimizedSeries.length
      )
        return null;

      const latestData = optimizedData[optimizedData.length - 1];
      const total = optimizedSeries.reduce(
        (sum, serie) => sum + (latestData[serie.dataKey] || 0),
        0
      );
    ...
    return (
      <GraphBase
        ...
        yAxis={{
          ...
          domain: areaConfigMerged.showBaseline
            ? [areaConfigMerged.baselineValue, 'auto']
            : undefined,
        }}
