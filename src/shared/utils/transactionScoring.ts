...
export function calculateTransactionScores(
...
function calculateFinancialScore(transaction: TransactionData): number {
  ...
  if (
    essentialCategories.some((cat) => category.name.toLowerCase().includes(cat))
  ) {
    score += 20;
  }
  ...
  if (
    discretionaryCategories.some((cat) =>
...
function calculateHealthScore(transaction: TransactionData): number {
  ...
  if (
    healthyKeywords.some((keyword) => merchant.toLowerCase().includes(keyword))
  ) {
    score += 30;
  }
  ...
  if (
    unhealthyKeywords.some((keyword) =>
      merchant.toLowerCase().includes(keyword)
    )
  ) {
    score -= 30;
...
function calculateEcoScore(transaction: TransactionData): number {
  ...
  // Special cases
  if (
    merchant.toLowerCase().includes('gas') ||
    merchant.toLowerCase().includes('fuel')
  ) {
    score = 20;
  }

  if (
    merchant.toLowerCase().includes('electric') ||
    merchant.toLowerCase().includes('renewable')
  ) {
    score = 85;
  }
