...
export function useTimeRange(
  ...
  const isValidRange = useCallback(
    (range: string): range is TimeRangeOption => {
  ...
  const setTimeRange = useCallback(
    (range: TimeRangeOption) => {
      if (opts.validateDates && !isValidRange(range)) {
        console.warn(
          `Invalid time range: ${range}. Using fallback: ${opts.fallbackRange}`
        );
  ...
  const getFilteredData = useCallback(
    <T extends Record<string, any>>(
      ...
      if (opts.validateDates) {
        ...
        const hasDateField =
          data.length > 0 && data[0][dateField] !== undefined;
        if (!hasDateField) {
  ...
  return useMemo(
    (): UseTimeRangeReturn => ({
      ...
      // Actions
      setTimeRange: opts.stabilizeCallbacks
        ? setTimeRange
        : context.setTimeRange,
      resetToDefault,

      // Data utilities
      getFilteredData: opts.memoizeData
        ? getFilteredData
