...
export const auditCardHoverEffects = (): CardHoverAuditResult => {
  ...
  const standardizedCount = standardCardHover.length + subtleCardHover.length + enhancedCardHover.length;
  ...
  const result: CardHoverAuditResult = {
    ...
    nonStandardCards: Array.from(oldHoverEffects, el => getElementIdentifier(el)),
    totalCards,
    compliancePercentage: totalCards > 0 ? Math.round((standardizedCount / totalCards) * 100) : 100
  };
  ...
  if (result.nonStandardCards.length > 0) {
    console.warn('[UICard-Refactor] Found cards with non-standard hover effects:', result.nonStandardCards);
  }
...
const getElementIdentifier = (element: Element): string => {
  ...
  if (className) {
    const relevantClasses = className.split(' ').filter(cls => 
...
export const logCardHoverApplication = (componentName: string, hoverType: 'standard' | 'subtle' | 'enhanced') => {
  console.info(`[UICard-Refactor] Applied ${hoverType} hover effect to ${componentName}`);
};
