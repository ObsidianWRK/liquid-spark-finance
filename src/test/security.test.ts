  ...
  describe('VueniInputSanitizer', () => {
    describe('sanitizeText', () => {
      ...
      it('should throw error for non-string input', () => {
        expect(() => VueniInputSanitizer.sanitizeText(123 as any)).toThrow(
    ...
    describe('sanitizeFinancialAmount', () => {
      it('should sanitize valid financial amounts', () => {
        expect(VueniInputSanitizer.sanitizeFinancialAmount('$1,234.56')).toBe(
          1234.56
        );
        expect(VueniInputSanitizer.sanitizeFinancialAmount('1234.567')).toBe(
          1234.57
        );
        expect(VueniInputSanitizer.sanitizeFinancialAmount(1234.567)).toBe(
          1234.57
      ...
      it('should handle negative amounts', () => {
        expect(VueniInputSanitizer.sanitizeFinancialAmount('-500.00')).toBe(
          -500.0
        );
        expect(VueniInputSanitizer.sanitizeFinancialAmount(-500.123)).toBe(
          -500.12
        );
      });
      ...
      it('should throw error for extremely large amounts', () => {
        expect(() =>
          VueniInputSanitizer.sanitizeFinancialAmount('1000000000001')
        ).toThrow('Financial amount exceeds maximum allowed value');
      });

      it('should throw error for invalid formats', () => {
        expect(() =>
          VueniInputSanitizer.sanitizeFinancialAmount('abc')
        ).toThrow('Invalid financial amount format');
        expect(() =>
          VueniInputSanitizer.sanitizeFinancialAmount('12.34.56')
        ).toThrow('Invalid financial amount format');
      });

      it('should throw error for non-finite numbers', () => {
        expect(() => VueniInputSanitizer.sanitizeFinancialAmount(NaN)).toThrow(
          'Invalid financial amount: not a finite number'
        );
        expect(() =>
          VueniInputSanitizer.sanitizeFinancialAmount(Infinity)
        ).toThrow('Invalid financial amount: not a finite number');
      });
    });
    ...
    describe('sanitizeTransactionDescription', () => {
      it('should sanitize and trim transaction descriptions', () => {
        ...
        const result =
          VueniInputSanitizer.sanitizeTransactionDescription(input);
        expect(result).toBe(
          '&lt;b&gt;Amazon Purchase&lt;&#x2F;b&gt; - groceries'
        );
      });
      ...
      it('should reject descriptions that are too long', () => {
        ...
        expect(() =>
          VueniInputSanitizer.sanitizeTransactionDescription(longInput)
        ).toThrow('Transaction description too long');
      });
    ...
    describe('sanitizePercentage', () => {
      ...
      it('should reject percentages outside valid range', () => {
        expect(() => VueniInputSanitizer.sanitizePercentage('-1')).toThrow(
          'Percentage must be between 0 and 100'
        );
        expect(() => VueniInputSanitizer.sanitizePercentage('101')).toThrow(
          'Percentage must be between 0 and 100'
        );
      });
    });
    ...
    describe('sanitizeInterestRate', () => {
      ...
      it('should reject extreme interest rates', () => {
        expect(() => VueniInputSanitizer.sanitizeInterestRate('-1')).toThrow(
          'Interest rate must be between 0% and 1000%'
        );
        expect(() => VueniInputSanitizer.sanitizeInterestRate('1001')).toThrow(
          'Interest rate must be between 0% and 1000%'
        );
      });
    });
    ...
    describe('sanitizeYear', () => {
      ...
      it('should reject years outside valid range', () => {
        const currentYear = new Date().getFullYear();
        expect(() => VueniInputSanitizer.sanitizeYear('1899')).toThrow(
          `Year must be between 1900 and ${currentYear + 100}`
        );
        expect(() =>
          VueniInputSanitizer.sanitizeYear(currentYear + 101)
        ).toThrow(`Year must be between 1900 and ${currentYear + 100}`);
      });

      it('should reject non-integer years', () => {
        expect(() => VueniInputSanitizer.sanitizeYear('2024.5')).toThrow(
    ...
    describe('sanitizeEmail', () => {
      it('should accept valid email addresses', () => {
        expect(VueniInputSanitizer.sanitizeEmail('test@example.com')).toBe(
          'test@example.com'
        );
        expect(VueniInputSanitizer.sanitizeEmail('  USER@DOMAIN.COM  ')).toBe(
          'user@domain.com'
        );
      });
      ...
      it('should reject invalid email formats', () => {
        ...
        expect(() => VueniInputSanitizer.sanitizeEmail('test@')).toThrow(
          'Invalid email format'
        );
        expect(() => VueniInputSanitizer.sanitizeEmail('@domain.com')).toThrow(
          'Invalid email format'
        );
      });
      ...
      it('should reject emails that are too long', () => {
        ...
        expect(() => VueniInputSanitizer.sanitizeEmail(longEmail)).toThrow(
    ...
    describe('sanitizePhoneNumber', () => {
      it('should format valid US phone numbers', () => {
        expect(VueniInputSanitizer.sanitizePhoneNumber('1234567890')).toBe(
          '(123) 456-7890'
        );
        expect(VueniInputSanitizer.sanitizePhoneNumber('11234567890')).toBe(
          '+1 (123) 456-7890'
        );
        expect(VueniInputSanitizer.sanitizePhoneNumber('(123) 456-7890')).toBe(
          '(123) 456-7890'
        );
      });
      ...
      it('should reject invalid phone number formats', () => {
        expect(() =>
          VueniInputSanitizer.sanitizePhoneNumber('123456789')
        ).toThrow('Invalid phone number format');
        expect(() =>
          VueniInputSanitizer.sanitizePhoneNumber('123456789012')
        ).toThrow('Invalid phone number format');
      });
    });
  });
...
describe('Security - Monitoring Tests', () => {
  ...
  describe('VueniSecurityMonitor', () => {
    ...
    it('should include metadata in events', () => {
      VueniSecurityMonitor.logEvent('LOGIN_ATTEMPT', 'Failed login', {
        ip: '192.168.1.1',
      });
...
describe('Security - Secure Storage Tests', () => {
  ...
  describe('VueniSecureStorage', () => {
    ...
    it('should validate key naming conventions', () => {
      ...
      expect(() => VueniSecureStorage.setItem('invalid-key', testData)).toThrow(
        'Invalid financial data key format'
      );
    });

    it('should handle session-only storage', () => {
      const sensitiveData = { ssn: '123-45-6789' };
      VueniSecureStorage.setItem('vueni:sensitive', sensitiveData, {
        sessionOnly: true,
      });
    ...
    it('should handle session expiry', () => {
      const data = { test: 'data' };
      VueniSecureStorage.setItem('vueni:session-test', data, {
        sessionOnly: true,
      });
    ...
    it('should require confirmation for clearing all data', () => {
      expect(() => VueniSecureStorage.clear()).toThrow(
        'Data clear operation requires confirmation token'
      );
      expect(() => VueniSecureStorage.clear('WRONG_TOKEN')).toThrow(
        'Data clear operation requires confirmation token'
      );
    });
...
describe('Security Integration Tests', () => {
  it('should handle complete security workflow', () => {
    ...
    const sanitizedText = VueniInputSanitizer.sanitizeText(
      '<script>alert("xss")</script>'
    );
    const sanitizedAmount =
      VueniInputSanitizer.sanitizeFinancialAmount('$1,234.56');
    ...
    VueniSecureStorage.setItem('vueni:transaction', financialData, {
      sensitive: true,
    });
    ...
    VueniSecurityMonitor.logEvent(
      'SECURE_TRANSACTION',
      'Financial data processed securely'
    );
