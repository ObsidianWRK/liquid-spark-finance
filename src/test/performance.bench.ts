...
describe('Performance Benchmarks - WebGL and Liquid Glass Effects', () => {
  describe('Component Rendering Performance', () => {
    bench('SharedScoreCircle - Single render', () => {
      const { unmount } = render(
        React.createElement(SharedScoreCircle, { score: 85, type: 'health' })
      );
      unmount();
    });
    ...
    bench('SharedScoreCircle - Multiple renders (100x)', () => {
      for (let i = 0; i < 100; i++) {
        const { unmount } = render(
          React.createElement(SharedScoreCircle, {
    ...
    bench('GlassCard - Single render with backdrop blur', () => {
      const { unmount } = render(
        React.createElement(GlassCard, {}, 'Test Content')
      );
      unmount();
    });
    ...
    bench('TransactionWithScores - Single transaction render', () => {
      const { unmount } = render(
        React.createElement(TransactionWithScores, {
  ...
  describe('CSS Animation Performance', () => {
    ...
    bench('Multiple glass elements with animations', () => {
      ...
      for (let i = 0; i < 20; i++) {
        const element = document.createElement('div');
        element.className =
          'backdrop-blur-lg bg-gradient-to-br from-white/20 to-white/5 border border-white/30 rounded-2xl';
      ...
      if (ctx) {
        ...
        for (let frame = 0; frame < 60; frame++) {
          ...
          gradient.addColorStop(
            0,
            `rgba(99, 102, 241, ${0.1 + 0.1 * Math.sin(frame * 0.1)})`
          );
          gradient.addColorStop(
            0.5,
            `rgba(139, 92, 246, ${0.05 + 0.05 * Math.cos(frame * 0.08)})`
          );
          gradient.addColorStop(
            1,
            `rgba(59, 130, 246, ${0.08 + 0.08 * Math.sin(frame * 0.12)})`
          );
  ...
  describe('Mathematical Calculations Performance', () => {
    bench(
      'Financial calculator - Compound Interest (1000 calculations)',
      () => {
    ...
    bench('Complex financial scenario calculations', () => {
      for (let i = 0; i < 100; i++) {
        ...
        const freedomYears = calculateFinancialFreedomYears(
          futureValue,
          monthlyPayment * 12,
          0.04
        );
  ...
  describe('Data Processing Performance', () => {
    bench('Transaction scoring - Single transaction', () => {
      ...
      const financialScore = Math.max(
        0,
        Math.min(100, 90 - (transaction.amount / 500) * 10)
      );
    });
    ...
    bench('Complex data aggregation and analysis', () => {
      ...
      const categoryStats = Object.entries(byCategory).map(
        ([category, txns]) => ({
          ...
          count: txns.length,
          averageHealthScore:
            txns.reduce((sum, txn) => sum + txn.healthScore, 0) / txns.length,
          averageEcoScore:
            txns.reduce((sum, txn) => sum + txn.ecoScore, 0) / txns.length,
          averageFinancialScore:
            txns.reduce((sum, txn) => sum + txn.financialScore, 0) /
            txns.length,
        })
      );
