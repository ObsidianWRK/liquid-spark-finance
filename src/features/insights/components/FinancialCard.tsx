...
const FinancialCard: React.FC<FinancialCardProps> = ({ data }) => {
  ...
  const keyMetrics = [
    ...
    },
    {
      ...
      color: '#06b6d4',
      trend:
        data.billPaymentScore >= 95
          ? ('up' as const)
          : data.billPaymentScore >= 85
            ? ('stable' as const)
            : ('down' as const),
    },
  ];
  ...
  const insights = [
    {
      ...
      status:
        data.spendingRatio <= 70
          ? 'good'
          : data.spendingRatio <= 85
            ? 'warning'
            : 'danger',
    },
    {
  ...
  return (
    <div className="w-full">
      {/* Header */}
      <div className="bg-white/[0.02] rounded-vueni-lg border border-white/[0.08] p-4 sm:p-6 mb-4 sm:mb-6 backdrop-blur-md shadow-md card-hover-subtle">
      ...
      {/* Financial Insights */}
      <div className="bg-white/[0.02] rounded-vueni-lg border border-white/[0.08] p-4 sm:p-6 backdrop-blur-md shadow-md card-hover-subtle">
        <h4 className="text-lg sm:text-xl font-bold text-white mb-4">
          Financial Insights
        </h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {insights.map((insight, index) => (
            <div key={insight.title} className="p-4 rounded-vueni-lg bg-white/5">
