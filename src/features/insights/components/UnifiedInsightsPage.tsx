...
const calculateSummaryStats = (transactions: Transaction[], accounts: Account[]) => {
...
const ScoreDisplay = memo(({ scores }: { scores: { financial: number; health: number; eco: number } }) => (
  <div className="grid grid-cols-3 gap-4 mb-6">
    <div className="text-center">
      <div className="w-16 h-16 rounded-full bg-white/[0.05] border border-white/[0.08] flex items-center justify-center mb-2 mx-auto">
        <span className="text-white text-sm font-medium">{scores.financial}</span>
      </div>
      <p className="text-white/80 text-xs">Financial</p>
    </div>
    <div className="text-center">
      <div className="w-16 h-16 rounded-full bg-white/[0.05] border border-white/[0.08] flex items-center justify-center mb-2 mx-auto">
        <span className="text-white text-sm font-medium">{scores.health}</span>
      </div>
...
const MetricCard = memo(({ title, value, subtitle }: { title: string; value: string; subtitle?: string }) => (
...
const CategoryBreakdown = memo(({ categories }: { categories: Record<string, number> }) => {
    ...
    <div className="space-y-3">
      ...
      {sortedCategories.map(([category, amount]) => (
        <div key={category} className="flex justify-between items-center">
          ...
          <span className="text-white text-sm font-medium">${amount.toFixed(0)}</span>
        </div>
      ))}
    </div>
  );
...
export const UnifiedInsightsPage = memo<UnifiedInsightsPageProps>(({ 
  config, 
  className,
  'data-testid': testId = 'unified-insights-page'
}) => {
  ...
  const stats = useMemo(() => calculateSummaryStats(transactions, accounts), [transactions, accounts]);

  const gridCols = layout.columns === 1 ? 'grid-cols-1' : 
                   layout.columns === 2 ? 'grid-cols-1 md:grid-cols-2' :
                   'grid-cols-1 md:grid-cols-2 lg:grid-cols-3';

  const spacing = layout.spacing === 'tight' ? 'gap-3' :
                  layout.spacing === 'wide' ? 'gap-8' : 'gap-6';
  ...
  return (
    <main 
      ...
      data-testid={testId}
      className={cn(
        'w-full bg-black text-white p-6 space-y-6',
        className
      )}
    >
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white mb-2">Financial Insights</h1>
        <p className="text-white/70">
          {variant === 'comprehensive' ? 'Comprehensive financial overview' : 'Financial summary'}
        </p>
      </div>
      ...
      {features.showScores && (
        <Card className="bg-white/[0.02] border-white/[0.08] p-6">
          <h2 className="text-white text-lg font-semibold mb-4">Your Scores</h2>
      ...
      {features.showTrends && (
        <Card className="bg-white/[0.02] border-white/[0.08] p-6">
          ...
          <div className="text-white/60 text-center py-8">
            ...
            <p className="text-sm mt-2">Data for {dataSource.timeframe} timeframe</p>
          </div>
        </Card>
      )}
      ...
      {variant === 'comprehensive' && (
        <div className={cn('grid', 'grid-cols-1 lg:grid-cols-2', spacing)}>
          <Card className="bg-white/[0.02] border-white/[0.08] p-6">
            <h3 className="text-white text-lg font-semibold mb-4">Account Summary</h3>
            <div className="space-y-3">
              {accounts.slice(0, 4).map((account) => (
                <div key={account.id} className="flex justify-between items-center">
                  <div>
                    <p className="text-white text-sm font-medium">{account.nickname || account.type}</p>
          ...
          <Card className="bg-white/[0.02] border-white/[0.08] p-6">
            <h3 className="text-white text-lg font-semibold mb-4">Financial Health</h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-white/80 text-sm">Overall Score</span>
                <span className="text-white text-lg font-semibold">{scores.financial}/100</span>
              </div>
