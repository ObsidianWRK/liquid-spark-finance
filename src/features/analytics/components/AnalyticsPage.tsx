...
const MindfulnessVsSpending = React.lazy(() => import('./health/MindfulnessVsSpending'));
const CaloricBalanceImpulseBuys = React.lazy(() => import('./health/CaloricBalanceImpulseBuys'));
...
export const AnalyticsPage: React.FC<AnalyticsPageProps> = ({
  ...
  const getDevicesByCompany = (companyId: string) => {
    return (
      deviceCompanies.find((company) => company.id === companyId)?.models || []
    );
  };

  const getConnectedDevicesByCompany = (companyId: string) => {
    return allConnectedDevices.filter(
      (device) => device.companyId === companyId
    );
  };

  const getTotalAvailableModels = () => {
    return deviceCompanies.reduce(
      (total, company) => total + company.models.length,
      0
    );
  };
  ...
  const connectedDevicesCount = useMemo(
    () =>
      allConnectedDevices.filter((device) => device.status === 'connected')
        .length,
    [allConnectedDevices]
  );
  ...
  const displayedDevices = useMemo(() => {
    ...
    const sortedDevices = [...allConnectedDevices].sort(
      (a, b) => priorityOrder[b.status] - priorityOrder[a.status]
    );
  ...
  const [unifiedData, setUnifiedData] = useState(
    unifiedDataManager.getSnapshot()
  );
    ...
    <PageContainer className={cn('max-w-7xl mx-auto p-4 sm:p-6 space-y-6', className)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-white/60">
            Comprehensive health, wealth, and transaction insights with
            correlations
          </p>
        </div>
      ...
      <div className="flex items-center gap-2">
        <Badge
          ...
          className={cn(
            'text-xs',
            dashboardData.dataQuality.overall >= 90
              ? 'border-green-400/20 text-green-400'
              : dashboardData.dataQuality.overall >= 70
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex flex-col text-xs text-white/50">
              <span>
                {connectedDevicesCount} of {allConnectedDevices.length} user
                devices connected
              </span>
              <span>
                {deviceCompanies.length} brands • {getTotalAvailableModels()}{' '}
                models available
              </span>
            </div>
            <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <UniversalCard variant="glass" interactive className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge
              variant="outline"
              className="border-green-400/20 text-green-400 text-xs"
            >
              Excellent
            </Badge>
          </div>
          <div className="text-3xl font-bold text-white mb-2">
            {unifiedData.health.wellnessScore}
          </div>
          ...
          <div className="mt-3 space-y-1">
            <div className="flex justify-between text-xs">
              ...
              <span className="text-white">
                {unifiedData.health.heartRateVariability}ms
              </span>
            </div>
            <div className="flex justify-between text-xs">
              ...
              <span
                className={cn(
                  'text-xs',
                  unifiedData.health.stressLevel > 60
                    ? 'text-red-400'
                    : unifiedData.health.stressLevel > 40
                      ? 'text-yellow-400'
                      : 'text-green-400'
                )}
              >
                {unifiedData.health.stressLevel > 60
                  ...
                  : unifiedData.health.stressLevel > 40
                    ? 'Medium'
                    : 'Low'}{' '}
                ({unifiedData.health.stressLevel})
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-white/60">Sleep</span>
              <span className="text-white">
                {unifiedData.health.sleepHours}h
              </span>
            </div>
          </div>
        </UniversalCard>
        ...
        <UniversalCard variant="glass" interactive className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge
              variant="outline"
              className="border-yellow-400/20 text-yellow-400 text-xs"
            >
              Good
            </Badge>
          </div>
          <div className="text-3xl font-bold text-white mb-2">
            {Math.round(
              (unifiedData.wealth.savingsRate +
                (100 - unifiedData.wealth.debtToIncomeRatio)) /
                2
            )}
          </div>
          <div className="flex items-center gap-2 text-sm">
          ...
          <div className="mt-3 space-y-1">
            <div className="flex justify-between text-xs">
              ...
              <span className="text-white">
                ${(unifiedData.wealth.netWorth / 1000).toFixed(1)}K
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-white/60">ROI</span>
              <span className="text-blue-400">
                {unifiedData.wealth.portfolioReturns.toFixed(1)}%
              </span>
            </div>
            <div className="flex justify-between text-xs">
        ...
        <UniversalCard variant="glass" interactive className="p-4">
          ...
          <div className="text-3xl font-bold text-white mb-2">
            {Math.round(
              ((Math.abs(unifiedData.correlations.stressSpending) +
                Math.abs(unifiedData.correlations.sleepROI) +
                Math.abs(unifiedData.correlations.activityDecisions)) /
                3) *
                100
            )}
          </div>
          ...
          <div className="mt-3 space-y-1">
            <div className="flex justify-between text-xs">
              ...
              <span
                className={cn(
                  'text-xs',
                  unifiedData.correlations.stressSpending > 0
                    ? 'text-orange-400'
                    : 'text-blue-400'
                )}
              >
                {unifiedData.correlations.stressSpending > 0 ? '+' : ''}
                {unifiedData.correlations.stressSpending.toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              ...
              <span
                className={cn(
                  'text-xs',
                  unifiedData.correlations.sleepROI > 0
                    ? 'text-green-400'
                    : 'text-red-400'
                )}
              >
                {unifiedData.correlations.sleepROI > 0 ? '+' : ''}
                {unifiedData.correlations.sleepROI.toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              ...
              <span
                className={cn(
                  'text-xs',
                  unifiedData.correlations.activityDecisions > 0
                    ? 'text-blue-400'
                    : 'text-red-400'
                )}
              >
                {unifiedData.correlations.activityDecisions > 0 ? '+' : ''}
                {unifiedData.correlations.activityDecisions.toFixed(2)}
              </span>
            </div>
          </div>
        </UniversalCard>
          ...
          <div className="space-y-4">
            ...
            <div className="mt-4 p-3 bg-white/[0.02] rounded-lg">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-white">
                  {unifiedData.health.heartRate} bpm
                </span>
              </div>
            ...
            <div className="mt-4 p-3 bg-white/[0.02] rounded-lg">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-green-400">
                  {unifiedData.health.sleepScore}/100
                </span>
              </div>
              <div className="flex justify-between text-xs mt-1">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-white/[0.02] rounded-lg">
              <div className="flex justify-between items-center mb-2">
                ...
                <Badge
                  variant="outline"
                  className="border-red-400/20 text-red-400 text-xs"
                >
                  +127% spending
                </Badge>
              </div>
            ...
            <div className="p-3 bg-orange-400/10 rounded-lg border border-orange-400/20">
              ...
              <div className="text-xs text-white/80">
                Your stress levels have been elevated for 3 days. Consider
                meditation before making purchases.
              </div>
            </div>
          </div>
        </UniversalCard>
        ...
        <UniversalCard variant="glass" interactive className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-blue-400/10 rounded-lg border border-blue-400/20">
              ...
              <div className="text-xs text-white/80">
                Better sleep correlates with improved decision-making and
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-white/[0.02] rounded-lg">
              <div className="flex justify-between items-center mb-3">
                ...
                <Badge
                  variant="outline"
                  className="border-blue-400/20 text-blue-400 text-xs"
                >
                  Improving
                </Badge>
              </div>
            ...
            <div className="p-4 bg-white/[0.02] rounded-lg">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-white">
                    ${((unifiedData.wealth.netWorth - 3240) / 1000).toFixed(1)}K
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-white/60">15 days ago</span>
                  <span className="text-white">
                    ${((unifiedData.wealth.netWorth - 1650) / 1000).toFixed(1)}K
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-white/60">7 days ago</span>
                  <span className="text-white">
                    ${((unifiedData.wealth.netWorth - 720) / 1000).toFixed(1)}K
                  </span>
                </div>
                <div className="flex justify-between text-xs">
            ...
            <div className="p-4 bg-white/[0.02] rounded-lg">
              <div className="flex justify-between items-center mb-3">
                ...
                <Badge
                  ...
                  className="border-green-400/20 text-green-400 text-xs"
                >
                  {unifiedData.wealth.savingsRate > 15 &&
                  unifiedData.wealth.debtToIncomeRatio < 20
                    ? 'Strong'
                    : unifiedData.wealth.savingsRate > 10 ||
                        unifiedData.wealth.debtToIncomeRatio < 30
                      ? 'Good'
                      : 'Fair'}
                </Badge>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="text-center">
                  <div className="text-lg font-bold text-green-400">
                    {(unifiedData.wealth.debtToIncomeRatio / 100).toFixed(2)}
                  </div>
                  <div className="text-xs text-white/60">Debt-to-Income</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-purple-400">
                    {unifiedData.wealth.emergencyFundMonths}
                  </div>
                  <div className="text-xs text-white/60">Emergency Fund</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-yellow-400">
                    {unifiedData.family.creditScore}
                  </div>
