...
class WellnessEngineService {
  ...
  private calculateTrend(
    values: number[],
    type: 'stress' | 'wellness'
  ): 'rising' | 'falling' | 'stable' | 'improving' | 'declining' {
  ...
  private calculateInterventionLevel(
    ...
    if (stressIndex >= 85 || (stressIndex >= 70 && trend === 'rising'))
      return 'severe';
    if (stressIndex >= 70 || (stressIndex >= 60 && trend === 'rising'))
      return 'moderate';
    if (stressIndex >= 55 || (stressIndex >= 45 && trend === 'rising'))
      return 'mild';
    return 'none';
  }
    ...
    for (const trigger of triggers.filter((t) => t.enabled)) {
      ...
      const shouldTrigger =
        ...
        (trigger.condition === 'equal' &&
          Math.abs(value - trigger.threshold) < 2);
  ...
  private initializeEngine(): void {
    ...
    combineLatest([biometricStream.biometricState$, this._triggers$])
      .pipe(
        map(([biometricState, triggers]) => {
          const { reading, stressIndex, wellnessScore, isActive, devices } =
            biometricState;
          ...
          const stressTrend = this.calculateTrend(stressHistory, 'stress') as
            | 'rising'
            | 'falling'
            | 'stable';
          const wellnessTrend = this.calculateTrend(
            wellnessHistory,
            'wellness'
        ...
        tap((state) => {
          ...
          const timeDiff =
            Date.now() - new Date(state.lastReading || 0).getTime();
          if (timeDiff > 50) {
  ...
  public updateTrigger(
    triggerId: string,
    updates: Partial<WellnessTrigger>
  ): void {
    const currentTriggers = this._triggers$.value;
    this._triggers$.next(
