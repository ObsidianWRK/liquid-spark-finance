...
const FinancialPlanningPage = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'goals' | 'retirement' | 'debt' | 'planning'
  >('overview');
  ...
  const loadPlanningData = async () => {
    ...
    try {
      ...
      const healthData =
        await financialPlanningService.getFinancialHealthScore(familyId);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="responsive-padding-md space-y-6 sm:space-y-8">
                        ...
                        <div className="text-left sm:text-right w-full sm:w-auto">
                          <div className="flex sm:items-center gap-2 text-sm">
                            ...
                            <span className={getStatusColor(goal)}>
                              {goal.progress.percentComplete >= 100
                                ? 'Completed'
        ...
        {activeTab === 'retirement' && <RetirementTab familyId={familyId} />}

        {activeTab === 'debt' && <DebtPayoffTab familyId={familyId} />}

        {activeTab === 'planning' && <LifePlanningTab familyId={familyId} />}
      </div>
    </div>
  );
};
