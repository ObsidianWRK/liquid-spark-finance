...
export const AccountCard: React.FC<Props> = ({
  ...
  const topGradient =
    categoryColors[acct.category || 'CHECKING'] ||
  ...
  const clampedDelta = deltaRatio
    ? Math.max(Math.min(deltaRatio * 100, 999), -999)
    : null;
  const deltaColor =
    clampedDelta && clampedDelta < 0 ? 'text-red-400' : 'text-green-400';
  const DeltaIcon =
    clampedDelta && clampedDelta < 0 ? ArrowDownRight : ArrowUpRight;

  // Safely calculate utilization percentage and clamp
  const utilRatio =
    acct.category === 'CREDIT'
      ? safeRatio(Math.abs(acct.currentBalance), acct.utilPercent || 100)
      : null;
  const clampedUtil = utilRatio
    ? Math.max(Math.min(utilRatio * 100, 999), 0)
    : acct.utilPercent
      ? Math.max(Math.min(acct.utilPercent, 999), 0)
      : null;
        ...
        <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
          <div className="bg-white/5 rounded-vueni-lg p-3">
            ...
            <div className="text-white font-semibold truncate">
              {showBalance
                ? formatCurrency(acct.availableBalance ?? acct.currentBalance, {
        ...
        {acct.lastTransaction && (
          <div className="flex items-center gap-2 text-sm mb-4">
            ...
            <span className="flex-1 truncate text-white/80">
              {acct.lastTransaction.merchant}
            </span>
            <span
              className={cn(
                'flex-shrink-0',
                acct.lastTransaction.amount < 0
                  ? 'text-red-400'
                  : 'text-green-400'
              )}
            >
              {showBalance
                ? formatCurrency(acct.lastTransaction.amount, {
                    currency: acct.currency,
                  })
                : '••••'}
            </span>
