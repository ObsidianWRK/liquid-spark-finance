...
export const QuickAccessCard = React.memo<QuickAccessCardProps>(
  ({ account, variant = 'rail', showBalance = true, onSelect, className }) => {
    ...
    const {
      ...
      const normalizedType =
        account.accountType
          ?.toLowerCase()
          .replace(/\s+/g, '_')
          .replace('card', '') || 'checking';
      const typeKey = (
        normalizedType in accountTypeColors ? normalizedType : 'checking'
      ) as keyof typeof accountTypeColors;
      ...
      if (account.spendDelta?.amount) {
        ...
        trend =
          account.spendDelta.trend === 'up'
            ? 'up'
            : account.spendDelta.trend === 'down'
              ? 'down'
              : 'flat';
      }
