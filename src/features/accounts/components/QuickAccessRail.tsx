  ...
  ({
    ...
    const scrollRight = useCallback(() => {
      ...
      const newScrollLeft = Math.min(
        maxScroll,
        railRef.current.scrollLeft + cardWidth
      );
    ...
    useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
        if (!document.activeElement?.closest('[data-quick-access-rail]'))
          return;
          ...
          case 'ArrowRight':
            ...
            if (isMobile) {
              ...
              setCurrentIndex((prev) =>
                Math.min(accounts.length - 1, prev + 1)
              );
            }
            break;
        }
    ...
    const displayedAccounts = isMobile
      ? accounts
      : accounts.slice(0, maxVisibleDesktop);
