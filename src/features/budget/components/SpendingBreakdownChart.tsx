...
const SpendingBreakdownChart: React.FC<SpendingBreakdownChartProps> = ({
  ...
  const chartData = useMemo((): StackedBarDataPoint[] => {
    ...
    const mapCategory = (txCategory: string): TransactionCategory => {
      ...
      if (
        ['gas', 'uber', 'lyft', 'transportation', 'fuel', 'transit'].some((k) =>
      ...
      if (
        ['health', 'pharmacy', 'cvs', 'healthcare', 'medical', 'doctor'].some(
          (k) => categoryLower.includes(k)
        )
      ) {
        return 'healthcare';
      }
      if (
        ['utilities', 'electric', 'water', 'internet', 'phone', 'cable'].some(
          (k) => categoryLower.includes(k)
        )
      ) {
        return 'utilities';
      ...
      if (
        ['rent', 'mortgage', 'housing', 'property', 'hoa'].some((k) =>
          categoryLower.includes(k)
        )
      ) {
        return 'housing';
      ...
      if (
        ['debt', 'loan', 'payment', 'credit card'].some((k) =>
          categoryLower.includes(k)
        )
      ) {
      ...
      if (
        ['saving', 'investment', '401', 'ira', 'transfer'].some((k) =>
          categoryLower.includes(k)
        )
      ) {
        return 'savings';
  ...
  const chartSeries = useMemo(() => {
    ...
    const categoryTotals = Array.from(categories).map((category) => ({
      ...
      total: chartData.reduce(
        (sum, point) => sum + ((point[category] as number) || 0),
        0
      ),
    }));
    ...
    return categoryTotals.map(({ category }) => ({
      dataKey: category,
      label:
        CATEGORY_MAPPING[category as TransactionCategory]?.label || category,
      color:
        CATEGORY_MAPPING[category as TransactionCategory]?.color || '#8E8E93',
    }));
  }, [chartData]);
  ...
  const insights = useMemo(() => {
    ...
    const totalSpending = chartData.reduce(
      (sum, point) => sum + ((point.total as number) || 0),
      0
    );
    ...
    chartData.forEach((point) => {
      chartSeries.forEach((serie) => {
        ...
        categoryTotals[serie.dataKey] =
          (categoryTotals[serie.dataKey] || 0) + value;
      });
    });
    ...
    const momChange =
      lastMonth && secondLastMonth
        ? (((lastMonth.total as number) - (secondLastMonth.total as number)) /
            (secondLastMonth.total as number)) *
          100
        : 0;
    ...
    return {
      ...
      topCategory: {
        name:
          CATEGORY_MAPPING[topCategory.category as TransactionCategory]
  ...
  const handleCategoryClick = useCallback(
    (data: StackedBarDataPoint, categoryKey: string, value: number) => {
    ...
    return (
      <div className="bg-white/[0.02] rounded-vueni-lg border border-white/[0.08] p-8 text-center">
        ...
        <p className="text-white/60">
          Connect your accounts to see spending breakdowns
        </p>
      </div>
    );
  }
