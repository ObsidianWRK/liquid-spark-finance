...
const ThreeFundPortfolioCalculator = () => {
  ...
  const handleCalculate = () => {
    const result = calculateThreeFundPortfolioReturn(
      usReturn,
      intlReturn,
      bondReturn,
      years
  ...
  return (
    <div className="max-w-xl mx-auto p-4 space-y-4">
      <h1 className="text-xl font-bold text-white">
        Bogleheads 3-Fund Portfolio Calculator
      </h1>
      <p className="text-white/70 text-sm">
        Assumes 40% US Stocks, 20% International, 40% Bonds.
      </p>
      <div className="space-y-2">
        <label className="block text-sm text-white/80">
          ...
          <input
            type="number"
            value={usReturn}
            onChange={(e) => setUsReturn(+e.target.value)}
        ...
        <label className="block text-sm text-white/80">
          ...
          <input
            type="number"
            value={intlReturn}
            onChange={(e) => setIntlReturn(+e.target.value)}
            className="w-full p-2 rounded bg-white/5 text-white border border-white/10"
          />
        </label>
        <label className="block text-sm text-white/80">
      ...
      {totalGrowth !== null && (
        <div className="text-white mt-4">
          ...
          <span className="font-semibold">{totalGrowth}%</span>
        </div>
      )}
    </div>
  );
